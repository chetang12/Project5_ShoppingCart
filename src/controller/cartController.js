...
const updateCart = async (req, res) => {
    try {
        ...
        if (!(typeof removeProduct == "number")) {
            return res
                .status(400)
                .send({ status: false, message: "Provide the removeProduct Key Should be in number only" });
        }
        if (!(removeProduct==1 || removeProduct==0))
            return res.status(400).send({
                status: false,
                message: "RemoveProduct Key Should be Zero Or One",
            });
        ...
        if (!cartId) {
            return res
                .status(400)
                .send({ status: false, message: "Provide the carrId" });
        }
...
const deleteCart = async (req, res) => {
    try {
        ...
        if (!isCart) {
            return res
                .status(404)
                .send({ status: false, message: "This Cart is Already Deleted" });
        } else {
